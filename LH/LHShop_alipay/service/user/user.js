...
handle = {
	...
	updateInfo: function (callback) {
		...
		// });
				// 1.登录
		_fn.mayiLogin(function (loginInfo) {
			if (_fn.isErrorRes(loginInfo)) {
				callback && callback(loginInfo);
				return;
			}
			// 2.获取微信侧登录信息
			_fn.getMayiUserInfo(function (wxUserInfo) {
				if (_fn.isErrorRes(wxUserInfo)) {
					callback && callback(wxUserInfo);
					return;
				}
...
_fn = {
	...
	getMayiUserInfo:function(callback){
		my.getAuthUserInfo({
			success: function (res) {

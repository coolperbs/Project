...
category = require('../../views/category/category'),
    //分类
//activity = require('../../views/activity/activity'),//活动
cart = require('../../views/cart/cart'),
    //购物车
...
Page({
  ...
  onShow: function (options) {
    ...
    // 每次显示都刷新一次购物车
    // 这样保证在商详添加后在首页也能显示
    var viewName = swan.getStorageSync('homeView'),
        currentView = self.data.currentView || 'home';
    setTimeout(function () {
      self.setData({
        shops: swan.getStorageSync('shops')
      });
    }, 100);
    currentView = viewName ? viewName : currentView;
    swan.removeStorageSync('homeView');
  ...
  jump: function (e) {
    ...
    if (url.indexOf('/comment/comment') > -1) {
      // 多了后面改为switch
      this.closeTips();
...
_fn = {
  selectView: function (viewName, options) {
    ...
    for (var i = 0; i < list.length; ++i) {
      if (list[i].view == viewName) {
        swan.setNavigationBarTitle({ title: list[i].text });
        break;
      }
    }
  ...
  needCommentTips: function () {
    var time = swan.getStorageSync('time') || '{}';
  ...
  writeTime: function () {
    ...
    swan.setStorageSync('time', JSON.stringify(time));
  }
};
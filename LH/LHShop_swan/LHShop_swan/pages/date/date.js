...
Page({
  ...
  onShow() {
    ...
    currentSelect = swan.getStorageSync('datetime') || [];
  ...
  selectDay: function (e) {
    ...
    if (currentSelect.length == 2) {
      swan.setStorageSync('datetime', currentSelect);
      swan.navigateBack();
    }
  },
  ...
  calculateDays(year, month) {
    ...
    for (let i = 1; i <= thisMonthDays; i++) {
      ...
      if (currentSelect.length == 1 && currentSelect[0].year == year && currentSelect[0].month == month && currentSelect[0].day == i) {
        selected = true;
...
_fn = {
  ...
  renderSelectDay: function (caller) {
    var selectDays = swan.getStorageSync('datetime') || {}; // start end allday
    caller.setData({

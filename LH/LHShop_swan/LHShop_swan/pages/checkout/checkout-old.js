...
Page({
  ...
  goCoupons: function (e) {
    ...
    if (tradeInfo.modules.coupon && tradeInfo.modules.coupon.data && tradeInfo.modules.coupon.data.valid && tradeInfo.modules.coupon.data.valid.length > 0) {
    ...
    swan.navigateTo({ url: '../coupon/coupon' });
  },
...
_fn = {
  ...
  submitFull: function (caller, param) {
    ...
    service.trade.submit(submitParam, function (orderRes) {
      ...
      if (!orderRes || orderRes.code != '0000' || !orderRes.data || !orderRes.data.orderId) {
        isPaying = false;
        swan.showToast({ title: orderRes.msg });
        return;
      }
      _fn.showLoading(caller, function () {
        ...
        service.trade.pay({
          ...
          if (!payRes || payRes.code != '0000') {
            swan.showToast({ title: payRes.msg });
            isPaying = false;
            if (isIn) {
              swan.redirectTo({ url: '../orderdetail/orderdetail?orderid=' + orderId });
            }
          ...
          service.trade.wxPay({
            ...
            if (!wxRes || wxRes.code != '0000') {
              swan.showToast({ title: wxRes.msg });
              if (isIn) {
                swan.redirectTo({ url: '../orderdetail/orderdetail?orderid=' + orderId });
              }
              // 跳转到订单
  ...
  createWares: function (data) {
    var result = [],
        ...
        j,
        sku;
    if (!data || !data.tradeInfo || !data.tradeInfo.modules || !data.tradeInfo.modules.wares || !data.tradeInfo.modules.wares.data || !data.tradeInfo.modules.wares.data.units) {
      return result;
    }

    units = data.tradeInfo.modules.wares.data.units;
    for (i = 0; u = units[i]; ++i) {
      if (!u.skus || u.skus.length <= 0) {
        continue;
      }
  ...
  formatPrice: function (data) {
    ...
    if (data && data.modules && data.modules.wares && data.modules.wares.data && data.modules.wares.data.units && data.modules.wares.data.units.length) {
      for (k = 0; unit = data.modules.wares.data.units[k]; ++k) {
        if (!unit.skus || unit.skus.length < 1) {
          continue;
        }

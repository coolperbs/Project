...
_fn = {
  refreshNum: function (caller, res) {
    if (res && res.data) {
      swan.setStorageSync('cartNum', res.data.cartTotalNum || 0);
    }

    // 未登录的情况
    if (res && res.code == '1000') {
      swan.setStorageSync('cartNum', 0);
    }
    caller.setData({
      cartNum: swan.getStorageSync('cartNum')
    });
  },
  // 处理用户信息，单独打个标记，merge购物车，因为在结算的时候必须merge一次，成功后才下单
  // 处理绑定信息
  addTempId: function (param) {
    var isMerge = swan.getStorageSync(CFG.isMergeCart),
    ...
    param.tempId = swan.getStorageSync(CFG.tempId);
    return param;
  },
  callbackFilter: function (res) {
    var isMerged = swan.getStorageSync(CFG.isMergeCart);
    // 处理购物车merge数据
    if (!isMerged && res && res.data && res.data.marge) {
      swan.setStorageSync(CFG.isMergeCart, true);
    }
    return res;
  }

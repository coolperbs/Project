...
handle = {
  ...
  checkRes: function (res) {
    ...
    if (res.code != '0000' || !res.success) {
      swan.showToast({ title: res.msg || '系统错误', image: '' }); // 需要一个错误图片
      return false;
    }
    return true;
  },
  ...
  topToHome: function (viewName) {
    swan.setStorageSync('homeView', viewName + '');
    swan.reLaunch({ url: '../index/index' });
  },
  add: function (a, b) {
    var c, d, e;
    try {

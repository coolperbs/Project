...
var conf = swan.getExtConfigSync && swan.getExtConfigSync() ? swan.getExtConfigSync() : {};
...
var handle = {
  ...
  onShow: function (options) {
    ...
    upperuid = swan.getStorageSync('upperuid') || upperuid;
    swan.setStorageSync('upperuid', upperuid);
  ...
  bindUpperUid: function (upperuid) {
    var hasBind = swan.getStorageSync('hasBind'),
        userInfo = swan.getStorageSync('userinfo') || {};

    upperuid = upperuid || swan.getStorageSync('upperuid');
    ...
    ajax.query({
      ...
      if (res.code == '0000' || res.code == '1003') {
        swan.setStorageSync('hasBind', true);
      }
    });
  }
};

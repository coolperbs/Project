<template>
  <div class="home">
    ...
    <div class="nav">
      <div class="icon" v-for=" item in cats ">
        <img :src="item.logo"/>
        {{item.catName}}
      </div>
    </div>-->
    ...
    <div class="cont-list">
    <router-link :to="{ path : detailPath, query : { id : item.shopId + '-' + item.skuId } }" v-for="item in pageInfo.wares" :key="item.skuId" class="mod">
        <div class="pic">
        ...
        <div class="info">
          ...
          <div class="more clearfix">
            <div class="price">
              ...
              <span class="counter" v-if="userData.trader==1 && catId != 1">返:￥{{ fixPrice( item.brokeragePrice) }}</span>
            </div>
...
<script>
  ...
  export default {
    mounted: function () {
      ...
      self.catId = catId;
      self.detailPath = catId == 1 ? '/mdetail' : '/detail';
      // 达人关系绑定
      if (query.userId) {
  ...
  _fn = {
    getData: function (callback) {
      var id = localStorage.getItem( 'venderId' ) || 1,
          url = window.location.href.split( '#' )[0];
      ajax.get('/app/index', { venderId : id, catId : catId, signUrl : window.location.href }, function (res) {
        if (utils.isErrorRes(res)) {
          utils.showError(res.msg || '请求接口出错');
          return;
        }

...
function meta2paths(meta) {
  ...
  var baseurl = seajs.data.base.substring( 0, seajs.data.base.length - 1 );
  for (var i = 0, path, root, subArr; i < paths.length; i++) {
    path = paths[i];
    root = path[0];
    subArr = path[1];
    var pos = root.indexOf(baseurl);
    var flag = seajs.data.base && pos > -1;
    if (!flag) {
      continue;
    }
    path[0] = baseurl;
    var prefix = root.replace(baseurl, '') || '';
    if (prefix.indexOf('/') == 0) {
      prefix = prefix.substr(1);
    }
    for (var j = 0; j < subArr.length; j++) {
      subArr[j] = prefix + subArr[j];
      if(subArr[j].indexOf("/")==0){
        subArr[j] = subArr[j].substr(1);
      }
    }
  }
  return paths
...
function setHash(root, files) {
  ...
  if (files.length > 1 && exceedMax) {
    ...
    for (var i = 0, len = files.length; i < len; i++) {
      comboHash[root + files[i]] = seajs.data.version ? comboPath + '?version=' + seajs.data.version : comboPath;
    }
  }
}

        ...
        PageClass.prototype = {
            enter: function () {
                ...
                Layout.on('inited', function () {
                    ...
                    seajs.use(source, function () {
                        ...
                        if ( kayak.router.prevPath == kayak.router.currentPath ) {
                            removeClass = false;
                        }else if ( kayak.router.currentPos > kayak.router.prevPos ) {
                            self.jView.removeClass( 'forwardout forwardin backwardin' ).addClass( 'forwardin' );
                        } else if ( kayak.router.currentPos < kayak.router.prevPos ) {
                            self.jView.removeClass( 'forwardout forwardin backwardin' ).addClass( 'backwardin' );
                        }
                        // 切换动画时 关闭定时器
                        setTimeout( function() {
                            if ( removeClass ) {
                                self.jView.removeClass( 'forwardout forwardin backwardin backwardout' );
                            }
                        }, 590 );
                        self.show && self.show();
                    });
            ...
            exit: function () {
                ...
                if ( kayak.router.prevPath == kayak.router.currentPath ) {
                    ...
                    self.jView.removeClass( 'forwardout forwardin backwardin' ).addClass( 'forwardout' );
                } else if (  kayak.router.currentPos < kayak.router.prevPos  ) {
                    self.jView.removeClass( 'forwardout forwardin backwardin' ).addClass( 'backwardout' );
                }
                // 切换到动画时关闭定时器
                setTimeout( function() {
                    if ( removeClass ) {

var data = {
	errCode : '0000',
	errMsg : '',
	data : {
		name : 'asdf',
		modules : [{
			type : 'slider',
			data : [{
				img : 'http://img.dmall.com/mIndex/201612/f8d1c169-1229-4121-8b39-b997d4cef239_750H',
				link : '../active/active',
				selected : true
			},{
				img : 'http://img.dmall.com/mIndex/201611/8412c37b-754b-4e1e-8e2c-cb8653cd4d0b_750H',
				link : '../active/active',
				selected : false
			},{
				img : 'http://img.dmall.com/mIndex/201612/5388bc94-0340-45a6-911d-2e1ea6fd11ca_750H',
				link : '../active/active',
				selected : false
			}]
		},{
			type : 'entry',
			data : [{
				img : 'http://img.dmall.com/mIndex/201611/396da250-8251-43ca-b4bd-4eaf91dfad8c_240x240H',
				text : '早晚市',
				link : '../active/active'
			},{
				img : 'http://img.dmall.com/mIndex/201611/6dde70cf-7d85-4fde-ba27-274bb6ab3391_240x240H',
				text : 'TOP榜',
				link : '../active/active'
			},{
				img : 'http://img.dmall.com/mIndex/201612/85f9a3cc-4755-4244-add0-5dd3b0acb2fd_240x240H',
				text : '满99减50',
				link : '../active/active'
			},{
				img : 'http://img.dmall.com/mIndex/201611/a9080773-7253-4f4d-81e6-26b67759a90d_240x240H',
				text : '7折区',
				link : '../active/active'
			},{
				img : 'http://img.dmall.com/mIndex/201611/fe54eeec-71bd-4974-8d7c-b06e13264da3_240x240H',
				text : '美通卡',
				link : '../active/active'
			}]
		},{
			type : 'module-3',
			data : [{
				img : 'http://img.dmall.com/mIndex/201611/6ef404b8-e474-4614-90d0-c88248c044d6_750H',
				height : '70',
				width : '720'
			}]
		},{
			type : 'module-1',
			data : [{
				img : 'http://img.dmall.com/20161206/0ec7c6eb-b60a-42d4-acdb-d47ad7844a13_360x360H',
				title : 'Mixxtail魅夜 专业鸡尾酒 蓝色幻想鸡尾酒（露酒）310ml',
				price : '8.00',
				skuId : 123,
				storeId : 456,
				originPrice : '9.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161112/06aacf18-237a-4b70-a238-e64c235007e7_360x360H',
				title : '曼可顿面包 香蕉牛奶味 68g',
				price : '1.65',
				originPrice : '2.25',
				sku : '',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161112/fe0bc645-e1e5-48ef-a792-44a81a2ef66e_360x360H',
				title : '康师傅蜂蜜绿茶 500ml',
				price : '2.50',
				originPrice : '3.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161112/283de793-f07b-474c-b766-ba1bc2bfa34f_360x360H',
				title : '金锣玉米热狗肠 40g*10',
				price : '9.90',
				originPrice : '12.60',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161215/de13f732-009a-4bb5-9b01-27613592d0dd_360x360H',
				title : '旺仔优酪乳 132g*16/箱',
				price : '19.90',
				originPrice : '60.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161213/b467ed5e-996f-4840-8c3a-600c6dfda9a9_360x360H',
				title : '绿伞去污超人洗衣液 3kg',
				price : '38.90',
				originPrice : '19.45',
				link : '../detail/detail?id=1'
			}]
		},{
			type : 'module-3',
			data : [{
				img : 'http://img.dmall.com/mIndex/201611/6ef404b8-e474-4614-90d0-c88248c044d6_750H',
				height : '70',
				width : '720'
			}]
		},{
			type : 'module-1',
			data : [{
				img : 'http://img.dmall.com/20161206/0ec7c6eb-b60a-42d4-acdb-d47ad7844a13_360x360H',
				title : 'Mixxtail魅夜 专业鸡尾酒 蓝色幻想鸡尾酒（露酒）310ml',
				price : '8.00',
				skuId : 123,
				storeId : 456,
				originPrice : '9.00',
				link : '../detail/detail?id=2'
			},{
				img : 'http://img.dmall.com/20161112/06aacf18-237a-4b70-a238-e64c235007e7_360x360H',
				title : '曼可顿面包 香蕉牛奶味 68g',
				price : '1.65',
				originPrice : '2.25',
				sku : '',
				link : '../detail/detail?id=2'
			},{
				img : 'http://img.dmall.com/20161112/fe0bc645-e1e5-48ef-a792-44a81a2ef66e_360x360H',
				title : '康师傅蜂蜜绿茶 500ml',
				price : '2.50',
				originPrice : '3.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161112/283de793-f07b-474c-b766-ba1bc2bfa34f_360x360H',
				title : '金锣玉米热狗肠 40g*10',
				price : '9.90',
				originPrice : '12.60',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161215/de13f732-009a-4bb5-9b01-27613592d0dd_360x360H',
				title : '旺仔优酪乳 132g*16/箱',
				price : '19.90',
				originPrice : '60.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161213/b467ed5e-996f-4840-8c3a-600c6dfda9a9_360x360H',
				title : '绿伞去污超人洗衣液 3kg',
				price : '38.90',
				originPrice : '19.45',
				link : '../detail/detail?id=1'
			}]
		},{
			type : 'module-3',
			data : [{
				img : 'http://img.dmall.com/mIndex/201612/1c81ca94-3c79-453c-bef3-88d89d0c41ff_480x480H',
				link : '../detail/detail?sku=234',
				height : '360',
				width : '720'
			},{
				img : 'http://img.dmall.com/mIndex/201612/1c81ca94-3c79-453c-bef3-88d89d0c41ff_480x480H',
				link : '../detail/detail?sku=234',
				height : '360',
				width : '720'
			}]
		},{
			type : 'module-3',
			data : [{
				img : 'http://img.dmall.com/mIndex/201611/6ef404b8-e474-4614-90d0-c88248c044d6_750H',
				height : '70',
				width : '720'
			}]
		},{
			type : 'module-1',
			data : [{
				img : 'http://img.dmall.com/20161206/0ec7c6eb-b60a-42d4-acdb-d47ad7844a13_360x360H',
				title : 'Mixxtail魅夜 专业鸡尾酒 蓝色幻想鸡尾酒（露酒）310ml',
				price : '8.00',
				skuId : 123,
				storeId : 456,
				originPrice : '9.00',
				link : '../detail/detail?id=2'
			},{
				img : 'http://img.dmall.com/20161112/06aacf18-237a-4b70-a238-e64c235007e7_360x360H',
				title : '曼可顿面包 香蕉牛奶味 68g',
				price : '1.65',
				originPrice : '2.25',
				sku : '',
				link : '../detail/detail?id=2'
			},{
				img : 'http://img.dmall.com/20161112/fe0bc645-e1e5-48ef-a792-44a81a2ef66e_360x360H',
				title : '康师傅蜂蜜绿茶 500ml',
				price : '2.50',
				originPrice : '3.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161112/283de793-f07b-474c-b766-ba1bc2bfa34f_360x360H',
				title : '金锣玉米热狗肠 40g*10',
				price : '9.90',
				originPrice : '12.60',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161215/de13f732-009a-4bb5-9b01-27613592d0dd_360x360H',
				title : '旺仔优酪乳 132g*16/箱',
				price : '19.90',
				originPrice : '60.00',
				link : '../detail/detail?id=1'
			},{
				img : 'http://img.dmall.com/20161213/b467ed5e-996f-4840-8c3a-600c6dfda9a9_360x360H',
				title : '绿伞去污超人洗衣液 3kg',
				price : '38.90',
				originPrice : '19.45',
				link : '../detail/detail?id=1'
			}]
		}]
	}
}

module.exports = data; 